...
probs <- seq(1, length(time_stats_ann$TimePercentage), 1)  
...
write.csv(time_stats_all, "results/Cladophera_Shear_Stress_time_stats.csv")
...
for(j in 1: length(td)) {
  ...
  total_days <- total_days %>% 
    ...
    rename(TimePeriod = season,  DaysPerMonth = days_per_month_low) %>%
    mutate(Species = "Cladophora", Life_Stage = "N/A", Hydraulic = "Shear Stress", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(total_days_all,"results/Clad_Shear_Stress_total_days.csv")
