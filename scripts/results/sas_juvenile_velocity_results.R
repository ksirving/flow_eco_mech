...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    mutate(Species = "Santa Ana Sucker", Life_Stage = "Juvenile", Hydraulic = "Velocity", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/SAS_Velocity_Juvenile_time_stats.csv")
...
write.csv(total_days_all,"results/SAS_Velocity_Juvenile_total_days.csv")
