...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Santa Ana Sucker", Life_Stage = "Fry", Hydraulic = "Velocity", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/SAS_fry_Velocity_time_stats_updated_hyd.csv")
...
write.csv(total_days_all,"results/SAS_Fry_Velocity_total_days_updated_hyd.csv")
