...
length(ts) ## 219
ts
ts <- Filter(function(x) grepl("germ", x), ts)
...
for(p in 1: length(probs)) {
  ...
  time_stats_ann$Suitability_Class[p] <- if(time_stats_ann$TimePercentage[p] >=30 && 
                                             time_stats_ann$TimePercentage[p] <= 76) {
    paste("High") 
    
  } else if (time_stats_ann$TimePercentage[p] <=30 || 
             time_stats_ann$TimePercentage[p] >= 76) {
    paste("Low") 
...
write.csv(time_stats_all, "/Users/katieirving/Documents/git/flow_eco_mech/results/Willow_Germination_Depth_time_stats.csv")
...
td <- Filter(function(x) grepl("germ", x), td)
td <- Filter(function(x) grepl("depth", x), td)

td <- td[-c(2,4,6,9,13,16)]
td
...
for(p in 1: length(probs)) {
  ...
  total_days_ann$Suitability_Class[p] <- if(total_days_ann$DaysPerMonth[p] >= 9 &&
                                            total_days_ann$DaysPerMonth[p] <= 24) {
    paste("High") 
    
  } else if (total_days_ann$DaysPerMonth[p] <= 9 ||
    total_days_ann$DaysPerMonth[p] >= 24) {
...
write.csv(total_days_all, "/Users/katieirving/Documents/git/flow_eco_mech/results/Willow_Germination_Depth_total_days.csv")
