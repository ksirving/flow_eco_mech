...
save(all_data, file="output_data/F1_LA20_adult_depth_discharge_probability_time_series_all_columns.RData")
...
save(new_data, file="output_data/F1_LA20_adult_depth_discharge_probability_time_series_red_columns.RData")
...
save(new_data, file="output_data/F1_LA20_depth_adult_discharge_probs_2010_2017_TS.RData")


# probability as a function of discharge -----------------------------------


load( file="output_data/F1_LA20_depth_adult_discharge_probs_2010_2017_TS.RData")
head(new_data)

## plot
range(new_data$Q) ##  0.00 28466.28
range(new_data$prob_fit) ## -0.006869681  0.398843946
...
write.csv(limits, "output_data/F1_LA20_Q_limits.csv")

# plot discharge points ---------------------------------------------------

png("figures/Application_curves/Depth/LA20_adult_depth_prob_Q_thresholds.png", width = 500, height = 600)
...
ggplot(new_data, aes(x = Q, y=prob_fit)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") +
  labs(title = "LA20: Adult/Depth: Probability ~ Q",
       y = "Probability",
       x = "Q (cfs)") #+ theme_bw(base_size = 15)
...
sum(new_dataRx$Q >= newx2aR & new_dataRx$Q <= newx2bR)/length(new_dataRx$DateTime)*100
...
write.csv(melt_time, "output_data/F1_LA20_adult_depth_time_stats.csv")
...
png("figures/Application_curves/Depth/LA20_adult_depth_perc_time_above_threshold_annual.png", width = 500, height = 600)
...
png("figures/Application_curves/Depth/LA20_adult_depth_perc_time_above_threshold_winter.png", width = 500, height = 600)
...
ggplot(melt_time_winter, aes(x = year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA20: Time within discharge limit in relation to Depth (Winter)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
...
png("figures/Application_curves/Depth/LA20_adult_depth_perc_time_above_threshold_summer.png", width = 500, height = 600)
...
ggplot(melt_time_summer, aes(x = year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA20: Time within discharge limit in relation to Depth (Summer)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
...
write.csv(total_days, "output_data/F1_LA20_total_days.csv")
...
## save df
write.csv(melt_days, "output_data/F1_LA20_total_days_long.csv")

library(scales)

## plot all ts

png("figures/Application_curves/Depth/LA20_adult_depth_lob_rob_mc_no_days_within_Q.png", width = 500, height = 600)
...
png("figures/Application_curves/Depth/LA20_adult_depth_lob_rob_mc_no_days_within_Q_by_year.png", width = 500, height = 600)
...
ggplot(melt_days, aes(x =month_year, y=n_days)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA20: Number of days within discharge limit in relation to Depth: Mid Channel",
       y = "Number of days per Month",
...
png("figures/Application_curves/Depth/LA20_adult_depth_lob_rob_mc_no_days_within_Q_by_season.png", width = 500, height = 600)
