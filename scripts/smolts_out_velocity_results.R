...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    mutate(Species = "Steelhead", Life_Stage = "Smolt", Hydraulic = "Velocity", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/Steelhead_Smolt_Velocity_time_stats.csv")
...
for(j in 1: length(td)) {
  ...
  total_days <- total_days %>% 
    ...
    rename(TimePeriod = season,  DaysPerMonth = days_per_month) %>%
    mutate(Species = "Steelhead", Life_Stage = "Smolts", Hydraulic = "Velocity", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(total_days_all,"results/Steelhead_Smolt_Velocity_total_days.csv")
