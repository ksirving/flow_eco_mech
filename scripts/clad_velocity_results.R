...
length(ts) ## 219
ts
ts <- Filter(function(x) grepl("clad", x), ts)
...
write.csv(time_stats_all, "results/Cladophera_Velocity_time_stats.csv")
...
td <- Filter(function(x) grepl("clad", x), td)
...
write.csv(total_days_all,"results/Clad_Velocity_total_days.csv")
