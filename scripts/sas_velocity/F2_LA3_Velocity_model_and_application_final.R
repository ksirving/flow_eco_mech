...
png("figures/Application_curves/nodes/LA3_Velocity_Q.png", width = 500, height = 600)
...
png("figures/Application_curves/nodes/LA3_Velocity_TS.png", width = 500, height = 600)
...
save(all_data, file="output_data/F2_LA3_SAS_adult_velocity_discharge_probability_time_series_all_columns.RData")
names(all_data)
## keep columns velocity, datetime, Q date_num & prob_fit
new_data <- all_data[order(all_data$date_num),]

save(new_data, file="output_data/F2_LA3_SAS_adult_velocity_discharge_probability_time_series_red_columns.RData")
...
save(new_data, file="output_data/F2_LA3_SAS_velocity_adult_discharge_probs_2010_2017_TS.RData")
...
limits
write.csv(limits, "output_data/F2_LA3_Adult_Velocity_Q_limits.csv")
# plot discharge points ---------------------------------------------------
unique(new_data$variable)

labels <- c(vel_m_LOB = "Left Over Bank", vel_m_MC = "Main Channel", vel_m_ROB = "Right Over Bank")

png("figures/Application_curves/Depth/LA3_adult_velocity_prob_Q_thresholds.png", width = 500, height = 600)
...
ggplot(new_data, aes(x = Q, y=prob_fit)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") +
  labs(title = "LA3: Adult/Velocity: Probability ~ Q",
       y = "Probability",
       x = "Q (cfs)") #+ theme_bw(base_size = 15)
...
write.csv(melt_time, "output_data/F2_LA3_adult_velocity_time_stats.csv")
...
png("figures/Application_curves/Velocity/LA3_adult_velocity_perc_time_above_threshold_annual.png", width = 500, height = 600)
...
ggplot(melt_time_ann, aes(x = water_year, y=value)) +
  ...
  labs(title = "LA3: Time within discharge limit in relation to Velocity (Annual)",
       y = "Time (%)",
...
png("figures/Application_curves/Velocity/LA3_adult_velocity_perc_time_above_threshold_winter.png", width = 500, height = 600)
...
ggplot(melt_time_winter, aes(x = water_year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA3: Time within discharge limit in relation to Velocity (Non_critical)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
...
png("figures/Application_curves/Velocity/LA3_adult_velocity_perc_time_above_threshold_critical.png", width = 500, height = 600)
...
ggplot(melt_time_summer, aes(x = water_year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA3: Time within discharge limit in relation to Velocity (Critical)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
...
write.csv(total_days, "output_data/F2_LA3_adult_velocity_total_days.csv")
...
## save df
write.csv(melt_days, "output_data/F2_LA3_adult_velocity_total_days_long.csv")
...
## plot all ts
png("figures/Application_curves/Velocity/LA3_adult_velocity_lob_rob_mc_no_days_within_Q.png", width = 500, height = 600)
...
ggplot(melt_days, aes(x =month_year, y=n_days)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA3: Number of days within discharge limit in relation to Velocity",
       y = "Number of days per Month",
       x = "Year") #+ theme_bw(base_size = 15)
dev.off()

## plot by year
png("figures/Application_curves/Velocity/LA3_adult_velocity_lob_rob_mc_no_days_within_Q_by_year.png", width = 500, height = 600)
...
png("figures/Application_curves/VelocityLA3_adult_velocity_lob_rob_mc_no_days_within_Q_by_season.png", width = 500, height = 600)
