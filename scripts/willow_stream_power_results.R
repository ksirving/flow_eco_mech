...
ts <- Filter(function(x) grepl("stream", x), ts)
...
range(time_stats_ann$TimePercentage)
mean(time_stats_ann$TimePercentage)
...
write.csv(time_stats_all, "/Users/katieirving/Documents/git/flow_eco_mech/results/Willow_Adult_Stream_Power_time_stats.csv")
...
# td <- Filter(function(x) grepl("stream", x), td)
...
for(j in 1: length(td)) {
  ...
  total_days <- total_days %>% 
    ...
    rename(TimePeriod = season,  DaysPerMonth = days_per_water_year) %>%
    mutate(Species = "Willow", Life_Stage = "Adult", Hydraulic = "Stream Power", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(total_days_all,"/Users/katieirving/Documents/git/flow_eco_mech/results/Willow_Adult_Stream_Power_total_days.csv")
