...
for(n in 1: length(h)) {
  ...
  write.csv(limits, paste("output_data/F1_",NodeName,"_SAS_juvenile_depth_Q_limits.csv", sep=""))
  ...
  ggplot(all_data, aes(x = Q, y=prob_fit)) +
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") +
    labs(title = paste(NodeName, ": Juvenile/Depth: Probability ~ Q", sep=""),
         y = "Probability",
         x = "Q (cfs)") #+ theme_bw(base_size = 15)
  ...
  write.csv(melt_time, paste("output_data/F1_", NodeName, "_SAS_juvenile_depth_time_stats.csv", sep=""))
  ...
  ## save df
  write.csv(melt_days, paste("output_data/F1_", NodeName, "_SAS_juvenile_depth_total_days_long.csv", sep="") )
