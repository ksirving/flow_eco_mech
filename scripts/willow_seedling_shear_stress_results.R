...
setwd("/Users/katieirving/Documents/git/willow_model_application")
## time stats
...
for(p in 1: length(probs)) {
  ...
  time_stats_ann$Suitability_Class[p] <- if(time_stats_ann$Probability_Threshold[p] == "High" && 
                                            time_stats_ann$TimePercentage[p] >= 75) {
    paste("Low") 
    
  } else if (time_stats_ann$Probability_Threshold[p] == "Low" && time_stats_ann$TimePercentage[p] <= 25) {
    paste("High") 
    
  } else if (time_stats_ann$Probability_Threshold[p] == "Medium") {
    paste("Partial") 
  } else {
    paste("Partial")
  }
...
probs <- seq(1, length(time_stats_seas$Probability_Threshold), 1)  

for(p in 1: length(probs)) {
  
  time_stats_seas$Suitability_Class[p] <- if(time_stats_seas$Probability_Threshold[p] == "High.Seasonal" && 
                                            time_stats_seas$TimePercentage[p] >= 75) {
    paste("Low") 
    
  } else if (time_stats_seas$Probability_Threshold[p] == "Low.Seasonal" && time_stats_seas$TimePercentage[p] <= 25) {
    paste("High") 
    
  } else if (time_stats_seas$Probability_Threshold[p] == "Medium.Seasonal") {
    paste("Partial") 
  } else {
    paste("Partial")
  }
...
write.csv(time_stats_all, "/Users/katieirving/Documents/git/flow_eco_mech/results/Willow_Seedling_Shear_Stress_time_stats.csv")
...
probs <- seq(1, length(total_days_ann$Probability_Threshold), 1)  

for(p in 1: length(probs)) {
  
  total_days_ann$Suitability_Class[p] <- if(total_days_ann$Probability_Threshold[p] == "High" && 
                                            total_days_ann$DaysPerMonth[p] >= 21) {
    paste("Low") 
    
  } else if (total_days_ann$Probability_Threshold[p] == "Low" && total_days_ann$DaysPerMonth[p] <= 7) {
    paste("High") 
    
  } else if (total_days_ann$Probability_Threshold[p] == "Medium") {
    paste("Partial") 
  } else {
    paste("Partial")
  }
...
write.csv(total_days_all,"/Users/katieirving/Documents/git/flow_eco_mech/results/Willow_Seedling_Shear_Stress_total_days.csv")
