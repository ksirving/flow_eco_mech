...
hyd_vel <- hyd_vel %>% rename(Velocity = value)
head(hyd_vel)
...
save(new_data, file="output_data/C3_LA20_2_clad_velocity_discharge_probability_time_series_red_columns.RData")
...
save(new_data, file="output_data/C3_LA20_2_clad_velocity_discharge_probs_2010_2017_TS.RData")
...
range(new_dataL$prob_fit)

## Main channel curves

load(file="root_interpolation_function.Rdata")

ifelse(min(new_dataL$prob_fit)>0.75, "yes", "no")
...
newx3a
newx3a <- ifelse(min(new_data$prob_fit)>0.75, min(new_data$Q), newx3a)
...
newx3aL
newx3aL <- ifelse(min(new_dataL$prob_fit)>0.75, min(new_dataL$Q), newx3aL)
...
newx3aR
newx3aR <- ifelse(min(new_dataR$prob_fit)>0.75, min(new_dataR$Q), newx3aR)
...
limits
write.csv(limits, "output_data/C3_LA20_2_clad_velocity_Q_limits.csv")
# plot discharge points ---------------------------------------------------
unique(new_data$variable)

labels <- c(vel_m_LOB = "Left Over Bank", vel_m_MC = "Main Channel", vel_m_ROB = "Right Over Bank")

png("figures/Application_curves/Velocity/C3_LA20_2_clad_velocity_prob_Q_thresholds.png", width = 500, height = 600)
...
ggplot(new_data, aes(x = Q, y=prob_fit)) +
  ...
  labs(title = "LA20_2: Cladophora/Velocity: Probability ~ Q",
       y = "Probability",
...
write.csv(melt_time, "output_data/C3_LA20_2_clad_velocity_time_stats.csv")
...
png("figures/Application_curves/Velocity/C3_LA20_2_clad_velocity_perc_time_above_threshold_annual.png", width = 500, height = 600)
...
png("figures/Application_curves/Velocity/C3_LA20_2_clad_velocity_perc_time_above_threshold_winter.png", width = 500, height = 600)
...
png("figures/Application_curves/Velocity/C3_LA20_2_clad_velocity_perc_time_above_threshold_critical.png", width = 500, height = 600)
...
write.csv(total_days, "output_data/C3_LA20_2_clad_velocity_total_days.csv")
...
## save df
write.csv(melt_days, "output_data/C3_LA20_2_clad_velocity_total_days_long.csv")
...
## plot all ts
png("figures/Application_curves/Velocity/C3_LA20_2_clad_velocity_lob_rob_mc_no_days_within_Q.png", width = 500, height = 600)
...
## plot by year
png("figures/Application_curves/Velocity/C3_LA20_2_clad_velocity_lob_rob_mc_no_days_within_Q_by_year.png", width = 500, height = 600)
...
png("figures/Application_curves/Velocity/C3_LA20_2_clad_velocity_lob_rob_mc_no_days_within_Q_by_season.png", width = 500, height = 600)
