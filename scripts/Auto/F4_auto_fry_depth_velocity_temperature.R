...
for(n in 1: length(h)) {
  ...
  ## save out
  save(all_data, file=paste("output_data/F4_", NodeName, "_SAS_depth_fry_discharge_probs_2010_2017_TS.RData", sep=""))
  ...
  for(p in 1:length(positions)) {
    ...
    if(min(curve$y) ==0 && max(curve$y) ==0) {
      newx1a <- 0
    } else if(min(curve$y)>3 && min(curve$y)<10) {
      newx1a <- min(curve$x)
    } else {
      newx1a <- approx(x = curve$y, y = curve$x, xout = 3)$y
    }
    
    if(min(curve$y) ==0 && max(curve$y) ==0) {
      newx2a <- 0
    } else if(min(curve$y)>10) {
      newx2a <- min(curve$x)
    } else {
      newx2a <- approx(x = curve$y, y = curve$x, xout = 10)$y
    }
  ...
  write.csv(limits, paste("output_data/F4_",NodeName,"_SAS_fry_depth_Q_limits.csv", sep=""))
  
  
  file_name = paste("figures/Application_curves/Depth/", NodeName, "_SAS_fry_depth_prob_Q_thresholds.png", sep ="")
  ...
  ggplot(all_data, aes(x = Q, y=value)) +
    ...
    labs(title = paste(NodeName, ": Fry/Depth: Depth ~ Q", sep=""),
         y = "Probability",
  ...
  write.csv(melt_time, paste("output_data/F4_", NodeName, "_SAS_fry_depth_time_stats.csv", sep=""))
  ...
  melt_days <- melt_days %>%
    rename( n_days = value) %>%
    select(-variable) %>%
    mutate(Species ="SAS", Life_Stage = "Fry", Hydraulic = "Depth")

  
  ## save df
  write.csv(melt_days, paste("output_data/F4_", NodeName, "_SAS_fry_depth_total_days_long.csv", sep="") )
...
for(n in 1: length(h)) {
  ...
  ## save out
  save(all_data, file=paste("output_data/F4_", NodeName, "_SAS_velocity_fry_discharge_probs_2010_2017_TS.RData", sep=""))
  ...
  for(p in 1:length(positions)) {
    ...
    curve <- spline(new_data$Q, new_data$value,
                    xmin = min(new_data$Q), xmax = max(new_data$Q), ties = mean)
    min(curve$y)#>0.001 && min(curve$y)<0.15
    ## main channel values
    if(min(curve$y) ==0 && max(curve$y) ==0) {
      newx1a <- 0
    } else if(min(curve$y)>0.001 && min(curve$y)<0.15) {
      newx1a <- min(curve$x)
    } else {
    ...
    if(min(curve$y) ==0 && max(curve$y) ==0) {
      newx2a <- 0
   } else  if(min(curve$y)>0.15) {
      newx2a <- min(curve$x)
    } else {
      newx2a <- approx(x = curve$y, y = curve$x, xout = 0.15)$y
    }
  ...
  write.csv(limits, paste("output_data/F4_",NodeName,"_SAS_fry_velocity_Q_limits.csv", sep=""))
  
  
  file_name = paste("figures/Application_curves/Depth/", NodeName, "_SAS_fry_velocity_prob_Q_thresholds.png", sep ="")
  ...
  ggplot(all_data, aes(x = Q, y=value)) +
    ...
    geom_point(data = subset(hyd_vel, variable =="vel_m_MC"), aes(y=0.15, x=limits[2,2]), color="green") +
    
    geom_point(data = subset(hyd_vel, variable =="vel_m_LOB"), aes(y=0, x=limits[1,1]), color="green") +
    geom_point(data = subset(hyd_vel, variable =="vel_m_LOB"), aes(y=0.15, x=limits[2,1]), color="green") +
    
    geom_point(data = subset(hyd_vel, variable =="vel_m_ROB"), aes(y=0, x=limits[1,3]), color="green") +
    geom_point(data = subset(hyd_vel, variable =="vel_m_ROB"), aes(y=0.15, x=limits[2,3]), color="green") +
    
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") +
    labs(title = paste(NodeName, ": Fry: Velocity ~ Q", sep=""),
         y = "Velocity (m/s)",
         x = "Q (cfs)") #+ theme_bw(base_size = 15)
  ...
  write.csv(melt_time, paste("output_data/F4_", NodeName, "_SAS_fry_velocity_time_stats.csv", sep=""))
  ...
  melt_days <- melt_days %>%
    rename( n_days = value) %>%
    select(-variable) %>%
    mutate(Species ="SAS", Life_Stage = "Fry", Hydraulic = "Velocity")
  
  
  ## save df
  write.csv(melt_days, paste("output_data/F4_", NodeName, "_SAS_fry_velocity_total_days_long.csv", sep="") )
