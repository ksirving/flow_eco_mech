...
for(n in 1: length(h)) {
  ...
  load(file=paste("output_data/F1_", NodeName, "_SAS_depth_juvenile_discharge_probs_2010_2017_TS_updated_hyd.RData", sep=""))
  BaseData <- all_data
  ...
  ## save out
  # save(all_data, file=paste("output_data/F1_", NodeName, "_SAS_juvenile_depth_discharge_probability_updated_hyd_.RData", sep=""))
  ...
  melt_time <- melt_time %>% 
    ...
    mutate(Species ="SAS", Life_Stage = "Juvenile", Hydraulic = "Depth", Node = NodeName, Scenario =scenarios[s])
  ...
  melt_days <- melt_days %>%
    ...
    mutate(Species ="SAS", Life_Stage = "Juvenile", Hydraulic = "Depth", Scenario =scenarios[s])
  ...
  ## save df
  write.csv(melt_timex, paste("output_data/scenarios/F1_", NodeName, "_SAS_juvenile_depth_time_stats_updated_hyd_scenarios.csv", sep=""))
  
  write.csv(melt_days, paste("output_data/scenarios/F1_", NodeName, "_SAS_juvenile_depth_total_days_long_updated_hyd_scenarios.csv", sep="") )
