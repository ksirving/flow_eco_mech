...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    mutate(Species = "Santa Ana Sucker", Life_Stage = "Juvenile", Hydraulic = "Depth", Node = paste(stringx[2])) %>%
    distinct()
...
time_stats_seas <- time_statsx %>%
  filter(Probability_Threshold %in% crit_stats, TimePeriod == "critical") %>%
  mutate(bottom_type = ifelse(Node %in% concrete, "Concrete", "Soft")) %>%
  distinct()
...
write.csv(time_stats_all, "results/SAS_Depth_Juvenile_time_stats.csv")
...
write.csv(total_days_all,"results/SAS_Depth_Juvenile_total_days.csv")
