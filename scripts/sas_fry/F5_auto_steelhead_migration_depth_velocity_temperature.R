...
for(n in 1: length(h)) {
  ...
  ## save out
  save(all_data, file=paste("output_data/F1_", NodeName, "_SAS_depth_fry_discharge_probs_2010_2017_TS.RData", sep=""))
  ...
  for(p in 1:length(positions)) {
    ...
    if(min(curve$y)>3 && min(curve$y)<10) {
      newx1a <- min(curve$x)
    } else {
  ...
  write.csv(melt_time, paste("output_data/F1_", NodeName, "_SAS_fry_depth_time_stats.csv", sep=""))
  ...
  ## save df
  write.csv(melt_days, paste("output_data/F1_", NodeName, "_SAS_fry_depth_total_days_long.csv", sep="") )
...
for(n in 1: length(h)) {
  ...
  ## save out
  save(all_data, file=paste("output_data/F1_", NodeName, "_SAS_velocity_fry_discharge_probs_2010_2017_TS.RData", sep=""))
  ...
  for(p in 1:length(positions)) {
    ...
    ## main channel values
    if(min(curve$y)>0.001 && min(curve$y)<0.15) {
      newx1a <- min(curve$x)
    } else {
      newx1a <- approx(x = curve$y, y = curve$x, xout = 0.001)$y
    }
  ...
  write.csv(melt_time, paste("output_data/F1_", NodeName, "_SAS_fry_velocity_time_stats.csv", sep=""))
  ...
  ## save df
  write.csv(melt_days, paste("output_data/F1_", NodeName, "_SAS_fry_velocity_total_days_long.csv", sep="") )
